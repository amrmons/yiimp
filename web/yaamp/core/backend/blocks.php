...
//	debuglog("NEW BLOCK $coin->name $db_block->height");
	...
	$is_solo = getdbocount('db_workers',"algo=:algo and userid=:userid and id=:workerid and password like '%m=solo%'", 
	array(':algo'=>$db_block->algo,':userid'=>$db_block->userid,':workerid'=>$db_block->workerid));
...
function BackendBlockFind2($coinid = NULL)
{
	...
	foreach($coins as $coin)
	{
		...
		foreach($list['transactions'] as $transaction)
		{
			...
			else if ($coin->rpcencoding == 'MN') {
				$db_block->category = 'generated';
			}
